...
const int SPSSTEREO_DEFAULT_OUTER_ITERATION_COUNT = 2;
const int SPSSTEREO_DEFAULT_INNER_ITERATION_COUNT = 2;
...
/*add argument: lableSEED by SanYu 20. July 2017*/
void SPSStereo::compute(const int superpixelTotal,
						unsigned int *lableSEED,
						...
						std::string &tempOutPutFileName)
{
	...
	setInputData(leftImage, rightImage,tempOutPutFileName);
/*add argument: lableSEED by SanYu 20. July 2017*/
	initializeSegment(superpixelTotal, lableSEED);
...
void SPSStereo::setInputData(const png::image<png::rgb_pixel>& leftImage, const png::image<png::rgb_pixel>& rightImage, std::string &tempOutPutFileName) {
	setLabImage(leftImage);
	computeInitialDisparityImage(leftImage, rightImage, tempOutPutFileName);
}
...
void SPSStereo::computeInitialDisparityImage(const png::image<png::rgb_pixel>& leftImage, const png::image<png::rgb_pixel>& rightImage, std::string &tempOutPutFileName) {
	SGMStereo sgm;
	sgm.compute(leftImage, rightImage, initialDisparityImage_);

/****************added by San Yu 31 Jan. 2017********************/
png::image<png::gray_pixel_16> localDisparityImage;
localDisparityImage.resize(width_, height_);
for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			double estimatedDisparity = initialDisparityImage_[width_*y + x];
			if (estimatedDisparity <= 0.0 || estimatedDisparity > 255.0) {
				localDisparityImage.set_pixel(x, y, 0);
			} else {
				localDisparityImage.set_pixel(x, y, static_cast<unsigned short>(estimatedDisparity*outputDisparityFactor_ + 0.5));
			}
		}
	}
//std::string tempDisparityImageFilename = "tempOutPutFileName";
localDisparityImage.write(tempOutPutFileName);
/****************added by San Yu 31 Jan. 2017********************/

}

void SPSStereo::initializeSegment(const int superpixelTotal, unsigned int *lableSEED) {
	makeGridSegment(superpixelTotal, lableSEED);
	assignLabel();
	initialFitDisparityPlane();
}
/*update new information to slanted-plane by SanYu 20. July 2017*/
void SPSStereo::makeGridSegment(const int superpixelTotal, unsigned int *lableSEED) {
...
/*the new one*/
	segmentTotal_ = superpixelTotal;
	segments_.resize(segmentTotal_);
	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			labelImage_[width_*y + x] = lableSEED[width_*y + x];
			int segmentIndex = lableSEED[width_*y + x];
			segments_[segmentIndex].addPixel(x, y, inputLabImage_[width_*3*y + 3*x], inputLabImage_[width_*3*y + 3*x + 1], inputLabImage_[width_*3*y + 3*x + 2]);
		}
	}
