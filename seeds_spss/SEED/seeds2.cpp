...
SEEDS::SEEDS(IplImage* img, int nr_bins, int NR_SUPERPIXELS)
{
...
int SEEDS::go_down_one_level()
{
	...
	for (int x=0; x<nr_w[new_level]; x++)
		for (int y=0; y<nr_h[new_level]; y++)
		{
			...
			nr_partitions[seeds_top_level][p] += nr_partitions[old_level][y*nr_w[new_level]+x];
			//original nr_partitions[seeds_top_level][p] += nr_partitions[new_level][y*nr_w[new_level]+x];
		}
...
int SEEDS::runSEED(IplImage* img, char* input_file1)
{
 ...
 *******************************************/
	...
	// SEEDS INITIALIZE
	int nr_superpixels = this->NR_SUPERPIXELS;
	...
	this->initialize(seed_width, seed_height, nr_levels);


	clock_t begin = clock();


	this->update_image_ycbcr(ubuff);
	...
	sz = 3*width*height;
  	...
  	//printf("Draw Contours Around Segments\n");
   	DrawContoursAroundSegments(ubuff, this->labels[nr_levels-1], width, height, 0xff0000, false);//0xff0000 draws red contours
   	DrawContoursAroundSegments(output_buff, this->labels[nr_levels-1], width, height, 0xffffff, true);//0xff0000 draws white contours



  	string imageFileName = getNameFromPathWithoutExtension(string(input_file1));
  	imageFileName = "./" + imageFileName + "_labels_SEED.png";
  	//printf("Saving image %s\n",imageFileName.c_str());
  	SaveImage(ubuff, width, height, imageFileName.c_str());

  	imageFileName = getNameFromPathWithoutExtension(string(input_file1));
  	imageFileName = "./" + imageFileName + "_boundary_SEED.png";
  	//printf("Saving image %s\n",imageFileName.c_str());
  	SaveImage(output_buff, width, height, imageFileName.c_str());
  
  	string labelFileNameTxt = getNameFromPathWithoutExtension(string(input_file1));
  	labelFileNameTxt = "./" + labelFileNameTxt + "_SEED.seg";
  	this->SaveLabels_Text(labelFileNameTxt);
...
void SEEDS::readLabels(UINT* copyLabels)
{

	memcpy(copyLabels, this->labels[this->seeds_top_level], (this->width) * (this->height) * sizeof(UINT));
	
}

int SEEDS::nr_Superpixels()
{
	return this->count_superpixels();
}
